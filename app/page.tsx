...
import { Activity, CheckCircle2, FileImage, ShieldCheck, UploadCloud } from "lucide-react"
...
export default function Home() {
  ...
  return <main className="min-h-screen px-5 py-8 md:px-10">
    ...
    <section className="mx-auto grid max-w-6xl gap-10 py-14 lg:grid-cols-[1fr_420px] lg:items-center">
      ...
      <div className="rounded-2xl border border-[#d7e5e8] bg-white p-6 shadow-[0_20px_50px_rgba(18,48,58,0.08)]"><div className="mb-5 flex items-start justify-between"><div><h2 className="m-0 text-xl font-bold">Analyze an image</h2><p className="mt-1 text-sm text-[#617982]">PNG, JPG, or JPEG</p></div><FileImage className="text-[#087f8c]" size={22} /></div><label className="flex min-h-48 cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-[#b8d2d7] bg-[#f4f8fa] px-5 text-center transition hover:border-[#087f8c] hover:bg-[#edf7f8]"><UploadCloud className="mb-3 text-[#087f8c]" size={30} /><span className="font-semibold">{file ? file.name : "Choose a chest X-ray"}</span><span className="mt-1 text-sm text-[#617982]">or drop it here</span><input className="sr-only" type="file" accept="image/png,image/jpeg" onChange={(e) => setFile(e.target.files?.[0] || null)} /></label><button onClick={analyze} disabled={!file || status === "Analyzing image…"} className="mt-5 w-full rounded-lg bg-[#087f8c] px-4 py-3 font-bold text-white transition hover:bg-[#075e69] disabled:cursor-not-allowed disabled:opacity-45">{status === "Analyzing image…" ? "Analyzing…" : "Run screening"}</button><p className="mt-4 text-center text-sm text-[#617982]">{status}</p>{result && <div className="mt-4 rounded-lg border border-[#b8d2d7] bg-[#edf7f8] p-4"><p className="m-0 text-xs font-bold uppercase tracking-wider text-[#617982]">Model result</p><p className="mt-2 text-2xl font-bold">{result.prediction}</p><p className="mt-1 text-sm text-[#617982]">Confidence: {result.confidence}%</p></div>}</div>
    </section>
  </main>
}
